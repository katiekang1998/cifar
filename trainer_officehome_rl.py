...
def rl_loss(output, target_var, misspecification_cost=4):
    one_hot = nn.functional.one_hot(target_var.to(torch.int64), 66)
...
def main():
    ...
    run = wandb.init(project="officehome_rl", name=args.save_dir)
...
def validate(val_loader, model, label):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            #valculate cross entropy with output
            loss = rl_loss(output, target)
            ...
            abstain_ratio = ((output.argmax(axis=-1) == output.size(1)-1).sum()/input.size(0)).item()
            abstain_ratios.update(abstain_ratio, input.size(0))


            one_hot = nn.functional.one_hot(target.to(torch.int64), output.size(1))
            reward_all = (4+1)*one_hot - 4
            reward_all[:, -1] = 0
            best_actions = output.argmax(axis=-1)
            rewards.update(torch.mean(torch.gather(reward_all, -1, best_actions.unsqueeze(-1)).type(torch.DoubleTensor)), input.size(0))
        ...
        print('Test: [{0}/{1}]\t'
              ...
              'Accuarcy {acc.val:.4f} ({acc.avg:.4f})'
              'Abstain ratio {ar.val:.4f} ({ar.avg:.4f})'
              'Reward {rew.val:.4f} ({rew.avg:.4f})'.format(
                  i, len(val_loader), batch_time=batch_time, loss=losses, acc=accuracies, ar=abstain_ratios, rew=rewards))

        wandb.log({label+'/loss': float(losses.avg)})
        wandb.log({label+'/acc': float(accuracies.avg)})
        wandb.log({label+'/abstain_ratio': float(abstain_ratios.avg)})
        wandb.log({label+'/reward': float(rewards.avg)})
