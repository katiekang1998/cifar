...
def main():
    ...
    if not os.path.exists(os.path.join("data", args.save_dir)):
        ...
        os.makedirs(os.path.join("data", args.save_dir, "reps"))
    ...
    for corruption_type in corruption_types:
        valc_loaders = []
        valc_loaders.append(val_loader)
        ...
        if epoch % args.save_every == 0:
            for corruption_type_idx in range(5):
                for corruption_level in range(6):
                    save_name = "ctype{}_clevel{}_epoch{}".format(corruption_type_idx, corruption_level, epoch)
                    save_rep(valc_loaders_all[corruption_type_idx][corruption_level], model,  
                    os.path.join("data", args.save_dir, "reps", save_name))
...
def save_rep(val_loader, model, name):
    """
    Run evaluation
    """
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            rep2 = model._modules.get('module').get_representation2(input_var)
            rep2 = rep2.float()

            representations = np.concatenate((representations, rep2.cpu().numpy()), axis=0)
            outputs = np.concatenate((outputs, output.cpu().numpy()), axis=0)
            targets = np.concatenate((targets, target.cpu().numpy()), axis=0)

    np.savez(name, representations=representations, outputs=outputs, targets=targets)
